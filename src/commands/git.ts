...
import {
  GitIntelligence,
  CommitSuggestion,
} from "../domain/git/git-intelligence.js";
import { PRGenerator } from "../domain/git/pr-generator.js";
import {
  CodeReviewEngine,
  ReviewOptions,
} from "../domain/git/code-reviewer.js";
...
export async function generateCommit(
  ...
  try {
    ...
    if (status.staged.length === 0) {
      console.log(
        chalk.yellow("⚠️  No staged changes found. Run `git add` first."),
      );
...
export async function generatePR(
  ...
  try {
    ...
    try {
      ...
      const args = ["pr", "create", "--title", titleFlag, "--body", pr.body];
      if (options.draft) {
        args.push("--draft");
      }
      ...
      console.log(
        chalk.gray("Please create manually using the title and body above."),
      );
      return { success: true, title: pr.title, body: pr.body };
    }
  } catch (error) {
