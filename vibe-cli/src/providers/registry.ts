...
export interface ProviderCapability {
  ...
  chat(messages: Array<{ role: string; content: string }>, options?: ProviderOptions): Promise<ProviderResponse>;
...
export function getProviderByModel(modelId: string): ProviderInfo | undefined {
  return PROVIDER_REGISTRY.find(p => p.models.some(m => m.id === modelId));
}

export function getModelsByTier(tier: 'fast' | 'balanced' | 'reasoning' | 'max'): Array<{ provider: string; model: ModelInfo }> {
...
export function getFreeTierModels(): Array<{ provider: string; model: ModelInfo }> {
  return getModelsByTier('fast').filter(m => m.model.freeTier);
}

export function listProviders(): Array<{ id: string; name: string; requiresApiKey: boolean }> {
